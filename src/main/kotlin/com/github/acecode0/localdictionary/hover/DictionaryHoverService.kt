    ...
    override fun mouseMoved(event: EditorMouseEvent) {
        ...
        delayController.schedule {
            ApplicationManager.getApplication().executeOnPooledThread {
                val definitions = DictionaryDatabase.openBundled().use { database ->
                    IdentifierWords.split(identifier).mapNotNull { word ->
                        database.lookup(word)?.let { "$word: $it" }
                    }
                }
                if (definitions.isNotEmpty()) {
                    ApplicationManager.getApplication().invokeLater {
                        HintManager.getInstance().showInformationHint(editor, definitions.joinToString("\n"))
                    }
                }
            }
        }
